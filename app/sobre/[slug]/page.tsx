...
export default async function About({
    ...
    return (
        <>
            ...
            <section className='flex flex-col items-center justify-center pb-12 gap-5 md:gap-10'>
                <h2 className='text-3xl md:text-4xl lg:text-6xl font-medium text-wrap justify-self-center tracking-[2px] md:tracking-[5px] text-primary-foreground'>· Trabalhos ·</h2>
                <div className='flex flex-col sm:flex-row flex-wrap gap-4 sm:gap-7 p-3 sm:p-6 items-center justify-center bg-quaternary box-shadow-large w-full'>
                    {works.map((work, index) => {
                        return (
                            <Link key={work.title + index}
                                ...
                                className='bg-[image:var(--image-url)] bg-cover bg-center w-96 h-80 flex flex-col justify-end py-5 px-6 box-shadow-large max-w-full'>
                                <p className='text-tertiary-foreground font-medium text-xl md:text-4xl text-shadow-sm'>{work.title}</p>
                            </Link>
                        )
                    })}
                </div>

                {/* TO-DO: style each work presentation */}

                {/* <div
                    style={{ backgroundImage: `url(/assets/img/${works[1].media})` }}
                    className='h-screen w-full bg-cover bg-left relative flex items-center justify-end'
                >
                    <div className='bg-background-secondary flex flex-col gap-9 w-7/12 mr-16 p-5'>
                        <h3 className='text-[110px] font-medium text-secondary-foreground'>{works[1].title}</h3>
                        <p className='text-xl font-normal text-secondary-foreground'>{works[1].content}</p>
                    </div>
                    <p className='absolute bottom-8 right-12 font-medium text-sm text-secondary-foreground'>Foto: {works[0].imageCredit}</p>
                </div>
